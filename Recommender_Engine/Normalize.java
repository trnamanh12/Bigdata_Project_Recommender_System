...
public class Normalize {
    ...
    public static class NormalizeMapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            String[] movie_relation = value.toString().trim().split('\t'); // movie_relation[0] = movie_1:movie_2, movie_relation[1] = relation ( number of people who watched both movies)
            String[] movies = movie_relation[0].split(':');

            context.write(new Text(movies[0]), new Text(movies[1] + ':' + movie_relation[1]));
            // key = movie_1, value = movie_2:relation
        }
    }
    ...
    public static class NormalizeReducer extends Reducer<Text, Text, Text, Text> {
        ...
        public void reduce(Text key, Iterable<Text> values, Context context)
            ...
            while (value.iterator().hasNext()){
                ...
                sum += relation; 
                map.put(movie_relation[0], relation); // movie_relation[0] = movie_2, movie_relation[1] = relation
            }
